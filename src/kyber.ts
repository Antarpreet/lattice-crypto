...
const INVN = 7651; // 256 * 7651 = 1 mod q
const bitRev_psi_7681_256 = [
...
export default class Kyber {
    ...
    compress2d(modulus: number, q: number, v: number[]): number[] {
        ...
        const vector: number[] = this.copyOf(v.slice(), v.length);
    ...
    NTT(A: number[], n: number): number[] {
        ...
        for (let m = 1; m < n; m <<= 1) {
            ...
            for (let i = 0; i < m; i++) {
                const j1 = (i << 1) * t;
                const j2 = j1 + t - 1;
                const S = bitRev_psi_7681_256[m+i];
                for (let j = j1; j <= j2; j++) {
                    const U = NTT_A_COEFF[j];
                    const V = NTT_A_COEFF[j+t] * S;
                    NTT_A_COEFF[j] = (U + V) % this.q;
                    while (NTT_A_COEFF[j] < 0) {
                        NTT_A_COEFF[j] = NTT_A_COEFF[j] + this.q;
                    }
                    NTT_A_COEFF[j+t] = (U - V) % this.q;
                    while (NTT_A_COEFF[j+t] < 0) {
                        NTT_A_COEFF[j+t] = NTT_A_COEFF[j+t] + this.q;
                    }
                }
            }
        }
    ...
    INTT(A: number[], n: number): number[] {
        ...
        for (let m = n; m > 1; m >>= 1)	{
            ...
            for (let i = 0; i < h; i++) {
                const j2 = j1 + t - 1;
                const S = bitRev_psiInv_7681_256[h+i];
                for (let j = j1; j <= j2; j++) {
                    const U = NTT_A_COEFF[j];
        ...
        for (let j = 0; j < n; j++) {
            NTT_A_COEFF[j] = (NTT_A_COEFF[j] * INVN) % this.q;
        }
        return NTT_A_COEFF;
    }
